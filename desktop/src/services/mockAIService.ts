...
export const mockAIService = {
  recognizeItem: async (imageFile: File): Promise<AIData> => {
    // Simulate processing delay (2-4 seconds)
    await delay(2000 + Math.random() * 2000);
    
    // Simple mock: try to detect item type from filename or use random
    ...
    if (fileName.includes('laptop') || fileName.includes('macbook')) {
      ...
      itemType = 'watch';
    } else if (fileName.includes('drill') || fileName.includes('tool')) {
      itemType = 'tool';
    } else if (fileName.includes('guitar') || fileName.includes('fender')) {
      itemType = 'guitar';
    } else if (fileName.includes('camera') || fileName.includes('canon')) {
      itemType = 'camera';
    } else if (fileName.includes('ring') || fileName.includes('jewelry')) {
      itemType = 'jewelry';
    }
  ...
  searchOnline: async (itemName: string): Promise<AIData['similarItems']> => {
    await delay(1000);
    // Return mock similar items
    return [
      { title: `${itemName} - Similar Item 1`, price: 100, platform: 'eBay' },
      { title: `${itemName} - Similar Item 2`, price: 120, platform: 'Facebook' },
    ];
  },
};
